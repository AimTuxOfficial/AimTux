...
/*---- Settings ----*/
bool Settings::Spammer::enabled = true;
/*---- Settings ----*/
...
void Spammer::Tick ()
{
	if (Settings::Spammer::enabled)
	{
		...
		unsigned long long currentTime_ms = std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now().time_since_epoch ()).count();
