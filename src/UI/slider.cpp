...
void Slider::Draw ()
{
	...
	DrawFilledRectangle (LOC(0, 5), LOC(donglePosition_px, size.y - 5), Settings::UI::mainColor);
	DrawRectangle (LOC(0, 5), LOC(size.x, size.y - 5), background_color);

	DrawFilledRectangle (LOC (donglePosition_px - (dongleWidth / 2), 5), LOC (donglePosition_px + dongleWidth / 2, size.y-5), background_color);
	...
	DrawCenteredString (_text, normal_font, text_color, LOC (size.x / 2, size.y / 2 + 2));
}
