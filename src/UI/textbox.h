...
public:
	...
	std::string* strp;
	std::string shadow_text = "numberbox";

	NumberBox (std::string shadow, std::string* textref, Vector2D position, Vector2D size)
		: shadow_text(shadow)
		, strp(textref)
	{
		this->size = size;
	...
	bool GetPressedKeys (std::vector<ButtonCode_t>& pressedKeys)
	{
		...
		for (int i = KEY_0; i <= KEY_RIGHT; i++)
		{
			...
			if ((input->IsButtonDown (key) && (key >= KEY_0 && key <= KEY_9 || key == KEY_BACKSPACE)) || (input->IsButtonDown (key) && (key >= KEY_PAD_0 && key <= KEY_PAD_9 || key == KEY_BACKSPACE)) )
			{
				_isKeyPressed = true;
				pressedKeys.push_back (key);
			}
