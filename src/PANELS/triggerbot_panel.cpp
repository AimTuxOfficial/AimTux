...
{
	ts_triggerbot_enabled = new ToggleSwitch ("Enabled", LOC (10, 10), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::enabled);
	...
	ts_delay = new ToggleSwitch ("Enabled", BELOW (ba_delay), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Delay::enabled);
	...
	ts_friendly = new ToggleSwitch ("Friendly", BELOW (ba_filter), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::friendly);
	AddComponent (ts_friendly);

	ts_head = new ToggleSwitch ("Head", BELOW (ts_friendly), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::head);
	AddComponent (ts_head);

	ts_chest = new ToggleSwitch ("Chest", BELOW (ts_head), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::chest);
	AddComponent (ts_chest);

	ts_arms = new ToggleSwitch ("Arms", BELOW (ts_chest), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::arms);
	AddComponent (ts_arms);

	ts_legs = new ToggleSwitch ("Legs", BELOW (ts_arms), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::legs);
