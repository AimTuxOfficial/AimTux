	...
	: Panel::Panel (position, size)
{
	...
	ts_delay = new ToggleSwitchTip ("Enabled", BELOW (ba_delay), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Delay::enabled, "Whether to use a delay on the trigger bot");
	...
	ts_friendly = new ToggleSwitchTip ("Friendly", BELOW (ba_filter), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::friendly, "Whether to trigger on friendlies");
	ts_head = new ToggleSwitchTip ("Head", BELOW (ts_friendly), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::head, "Whether to trigger on head");
	ts_chest = new ToggleSwitchTip ("Chest", BELOW (ts_head), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::chest, "Whether to trigger on chest");
	ts_arms = new ToggleSwitchTip ("Arms", BELOW (ts_chest), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::arms, "Whether to trigger on arms");
	ts_legs = new ToggleSwitchTip ("Legs", BELOW (ts_arms), LOC((size.x - 20) / 6.75, 30), &Settings::Triggerbot::Filter::legs, "Whether to trigger on legs");
