...
void Visuals::RenderTab()
{
	...
	float w = 0.1f*ImGui::GetWindowWidth();
	ImGui::PushID("##VISUALS");
	...
	{;
		...
		{
			if (current_category == Category::ESP) //{{{ ESP
			{
				...
				ImGui::Separator();
				ImGui::Text(XORSTR("Informations"));
				ImGui::Separator();
			...
			else if (current_category == Category::LOCAL) //{{{ LOCAL
			{
			...
			else if (current_category == Category::WORLD) // {{{ WORLD
			{
				ImGui::Text(XORSTR("General"));
				ImGui::Separator();
				...
				ImGui::Columns(2, nullptr, false);
				{
					ImGui::Checkbox(XORSTR("Custom Skybox"), &Settings::SkyBox::enabled);
